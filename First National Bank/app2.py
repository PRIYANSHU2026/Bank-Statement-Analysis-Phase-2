...
# Configuration
st.set_page_config(
    page_title='Enhanced FNB Statement Analysis',
    page_icon='🏦',
    layout='wide',
...
class DatabaseManager:
    ...
    def __init__(self):
        self.db_path = 'fnb_financial_analysis.db'
        self.init_database()
    ...
    def create_user(self, user_id, name, email, password, financial_goals="", risk_tolerance="moderate",
        ...
        try:
            cursor.execute('''
                           INSERT INTO user_profiles (user_id, name, email, password_hash, financial_goals,
    ...
    def get_user_profile(self, user_id):
        ...
        if result:
            columns = ['id', 'user_id', 'name', 'email', 'password_hash', 'financial_goals', 'risk_tolerance',
...
class AdvancedAnalytics:
    ...
    def enhanced_loan_prediction(self, df):
        ...
        try:
            # Load training data
            training_data = pd.read_csv('fnb.csv')
...
def extract_bank_statement_metadata(pdf_path):
    """Extract metadata from FNB bank statement"""
...
def parse_pdf_enhanced(file):
    """Enhanced PDF parsing with better text extraction for FNB statements"""
    try:
...
def process_text_to_df_enhanced(text):
    """Enhanced text processing with better pattern recognition for FNB statements"""
    transactions = []

    # Improved FNB transaction pattern
    ...
    # Additional pattern for transactions with reference numbers
    ref_pattern = re.compile(
        r'(\d{2} \w{3})\s+'  # Date
        r'([^\d]+)\s+'  # Description
        r'(\d{8,})\s+'  # Reference number
        r'(-?\d{1,3}(?:,\d{3})*(?:\.\d{2})?)(Cr|Dr)?\s*'  # Amount
        r'(-?\d{1,3}(?:,\d{3})*(?:\.\d{2})?)(Cr|Dr)?'  # Balance
    )
    ...
    for line in text.split('\n'):
        ...
        match = transaction_pattern.search(line) or ref_pattern.search(line)
        if match:
            try:
                groups = match.groups()
                if len(groups) == 6:  # Ref pattern match
                    date_str, description, _, amount_str, cr_dr1, balance_str, _ = groups
                else:  # Standard pattern match
                    date_str, description, amount_str, cr_dr1, balance_str, _ = groups

                # Convert date to standard format (assuming current year)
                ...
                transactions.append([date_str, description, amount, balance])
...
def categorize_expense_enhanced(description):
    """Enhanced expense categorization using NLP for FNB transactions"""
    description_lower = description.lower()

    # Enhanced categorization with FNB-specific rules
    category_keywords = {
        'Salary/Income': ['salary', 'wage', 'income', 'payroll', 'refund', 'credit'],
        'Groceries': ['pnp', 'checkers', 'woolworths', 'spar', 'pick n pay', 'food', 'grocery'],
        'Transport': ['fuel', 'petrol', 'uber', 'bolt', 'taxi', 'transport', 'car payment'],
        'Utilities': ['electricity', 'water', 'municipal', 'rates', 'internet', 'phone', 'eskom'],
        'Entertainment': ['restaurant', 'movie', 'netflix', 'spotify', 'dstv', 'entertainment'],
        'Healthcare': ['medical', 'doctor', 'hospital', 'pharmacy', 'health', 'dischem', 'clicks'],
        'Shopping': ['retail', 'clothing', 'amazon', 'takealot', 'mr price', 'edgars'],
        'Investments': ['investment', 'shares', 'unit trust', 'retirement', 'easy equities'],
        'Insurance': ['insurance', 'medical aid', 'life cover', 'short term', 'outsurance'],
        'POS Purchases': ['pos purchase', 'card purchase', 'debit order'],
        'Payments': ['payment to', 'fnb app rtc pmt', 'internet pmt', 'debit order'],
        'Bank Charges': ['service fee', 'monthly fee', 'bank charge', 'commission'],
        'Cash Transactions': ['atm cash', 'cash deposit', 'withdrawal', 'fnb atm'],
        'Cellular': ['airtime', 'data', 'vodacom', 'mtn', 'cell c', 'telkom'],
        'Interest': ['interest', 'bank interest'],
        'Failed Transactions': ['unsuccessful', 'declined', 'failed', 'item paid no funds']
    }
...
def main():
    ...
    with st.sidebar:
        st.title("🏦 FNB Financial Analysis")
    ...
    if st.session_state.user_profile is None:
        st.markdown("""
        # 🏦 Enhanced FNB Bank Statement Analysis

        ### Welcome to the next generation of financial analysis for FNB customers!
    ...
    st.title(f"🏦 FNB Financial Analysis Dashboard - {st.session_state.user_profile['name']}")

    # File upload section
    st.markdown("### 📄 Upload Your FNB Bank Statement")
    uploaded_file = st.file_uploader(
        "Choose a PDF bank statement",
        type="pdf",
        help="Upload your FNB bank statement in PDF format for analysis"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.spinner("🔍 Parsing bank statement..."):
                ...
                # Extract metadata
                account_holder_name, closing_balance = extract_bank_statement_metadata(temp_file_path)
            ...
            st.info("Please ensure your PDF is a valid FNB bank statement and try again.")
