...
# Configuration
st.set_page_config(
    page_title='Enhanced Nedbank Statement Analysis',
    page_icon='🏦',
    layout='wide',
...
class DatabaseManager:
    ...
    def __init__(self):
        self.db_path = 'nedbank_financial_analysis.db'
        self.init_database()
...
class AdvancedAnalytics:
    ...
    def enhanced_loan_prediction(self, df):
        ...
        try:
            # Load training data
            training_data = pd.read_csv('nedbank.csv')
            ...
            # Prepare feature vector - using only columns that exist in nedbank.csv
            ...
            # Train Random Forest model (fallback if XGBoost not available)
...
def extract_bank_statement_metadata(text):
    """Extract metadata from Nedbank statement"""
...
def parse_pdf_enhanced(file):
    """Enhanced PDF parsing with better text extraction for Nedbank statements"""
    try:
...
def process_text_to_df_enhanced(text):
    """Enhanced text processing with better pattern recognition for Nedbank statements"""
    transactions = []

    # Nedbank transaction pattern
...
def categorize_expense_enhanced(description):
    """Enhanced expense categorization using NLP for Nedbank transactions"""
    description_lower = description.lower()

    # Enhanced categorization with Nedbank-specific rules
    category_keywords = {
        ...
        'Bank Charges': ['service fee', 'monthly fee', 'bank charge', 'commission', 'fees'],
        'Cash Transactions': ['atm cash', 'cash deposit', 'withdrawal', 'nedbank atm'],
...
def main():
    ...
    with st.sidebar:
        st.title("🏦 Nedbank Financial Analysis")
    ...
    if st.session_state.user_profile is None:
        st.markdown("""
        # 🏦 Enhanced Nedbank Statement Analysis

        ### Welcome to the next generation of financial analysis for Nedbank customers!
    ...
    st.title(f"🏦 Nedbank Financial Analysis Dashboard - {st.session_state.user_profile['name']}")

    # File upload section
    st.markdown("### 📄 Upload Your Nedbank Statement")
    uploaded_file = st.file_uploader(
        "Choose a PDF bank statement",
        type="pdf",
        help="Upload your Nedbank statement in PDF format for analysis"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("Please ensure your PDF is a valid Nedbank statement and try again.")
