...
# Configuration
st.set_page_config(
    page_title='Enhanced Standard Bank Statement Analysis',
    page_icon='🏦',
    layout='wide',
...
class DatabaseManager:
    ...
    def __init__(self):
        self.db_path = 'standardbank_financial_analysis.db'
        self.init_database()
...
class AdvancedAnalytics:
    ...
    def enhanced_loan_prediction(self, df):
        ...
        try:
            # Load training data
            training_data = pd.read_csv('standardbank.csv')
            ...
            # Prepare feature vector - using Standard Bank specific columns
            ...
            y_train = training_data['Loan_Eligibility']
...
def extract_bank_statement_metadata(text):
    """Extract metadata from Standard Bank statement"""
...
def parse_pdf_enhanced(file):
    """Enhanced PDF parsing with better text extraction for Standard Bank statements"""
    try:
...
def process_text_to_df_enhanced(text):
    """Enhanced text processing with better pattern recognition for Standard Bank statements"""
    transactions = []

    # Standard Bank transaction pattern
...
def categorize_expense_enhanced(description):
    """Enhanced expense categorization using NLP for Standard Bank transactions"""
    description_lower = description.lower()

    # Enhanced categorization with Standard Bank-specific rules
    category_keywords = {
        'Salary': ['salary', 'wage', 'income', 'payroll'],
        'Bank Fees': ['fee', 'charge', 'service', 'cost', 'monthly account fee'],
        'Mobile': ['mtn prepaid', 'vodacom', 'airtime', 'data', 'topup'],
        'Utilities': ['electricity', 'water', 'municipal', 'rates', 'eskom'],
        'Payments': ['payment', 'transfer', 'debit order', 'immediate trf', 'digital payment'],
        'Shopping': ['store', 'shop', 'retail', 'takealot', 'checkers', 'woolworths'],
        'Transport': ['uber', 'taxi', 'fuel', 'petrol', 'transport'],
        'Cash': ['atm', 'cash', 'withdrawal', 'deposit'],
        'Investments': ['investment', 'shares', 'unit trust'],
        'Insurance': ['insurance', 'medical aid', 'life cover'],
        'Entertainment': ['netflix', 'dstv', 'movie', 'restaurant'],
...
def main():
    ...
    with st.sidebar:
        st.title("🏦 Standard Bank Financial Analysis")
    ...
    if st.session_state.user_profile is None:
        st.markdown("""
        # 🏦 Enhanced Standard Bank Statement Analysis

        ### Welcome to the next generation of financial analysis for Standard Bank customers!
    ...
    st.title(f"🏦 Standard Bank Financial Analysis Dashboard - {st.session_state.user_profile['name']}")

    # File upload section
    st.markdown("### 📄 Upload Your Standard Bank Statement")
    uploaded_file = st.file_uploader(
        "Choose a PDF bank statement",
        type="pdf",
        help="Upload your Standard Bank statement in PDF format for analysis"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("Please ensure your PDF is a valid Standard Bank statement and try again.")
